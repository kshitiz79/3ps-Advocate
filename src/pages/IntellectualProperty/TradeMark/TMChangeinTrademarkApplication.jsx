...
const TMChangeinTrademarkApplication = () => {
    ...
    if (data?.success && data.data) {
      ...
      const filteredProduct = data.data.find(
        (product) =>
          ...
          "TM-M/TM-P (Change in Trademark Application)".toLowerCase().trim()
      );
  ...
  if (!pageData) {
    return (
      <p className="text-center text-gray-500 mt-10">
        No product data available for "TM-M/TM-P (Change in Trademark Application)".
      </p>
    );
  }
...
export default TMChangeinTrademarkApplication